...
Slider.prototype.addEvent = function(){
	...
	let $left_btn = $(this.$box.selector+" .left_btn");
	$left_btn.click(function(){
		that.left();
	});

	let $right_btn = $(this.$box.selector+" .right_btn");
...
Slider.prototype.autoPlay = function(){	
	...
	this.myTimer = setInterval(()=>{
		...
		//3、改变外观
		this.showImg_rTol(outIndex,this.currIndex);
	},this.timeSpace);	
}
...
Slider.prototype.left=function(){//2
	//1、改变数据（图片序号）
	let outIndex = this.currIndex;
	this.currIndex=outIndex - 1;//2
	//2、边界处理
	if(this.currIndex<0){
		this.currIndex = this.imgs.length-1;
	}
	//3、改变外观
	this.showImg_lTor(outIndex,this.currIndex);
}

Slider.prototype.right=function(){//2
	//1、改变数据（图片序号）
	let outIndex = this.currIndex;
	this.currIndex=outIndex + 1;//2
	//2、边界处理
	...
	//outIndex:淡出的那张图片的序号
	//inIndex:淡入的那张图片的序号
Slider.prototype.showImg_rTol=function(outIndex,inIndex){
	//3、改变外观
	//1)、改图片
...
Slider.prototype.showImg_lTor=function(outIndex,inIndex){
	//3、改变外观
	//1)、改图片
	let $img =this.$box.children("img");
	$img.eq(inIndex).css({"left":-1*this.width}).animate({
		"left":0
	},300);
	$img.eq(outIndex).animate({
		"left":1*this.width
